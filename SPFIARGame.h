...
// Number of directions in which a location can be the left-most/top-most location.
#define SP_FIAR_GAME_N_DIRECTIONS 4
#define SP_FIAR_GAME_NO_SPAN ((-1)*SP_FIAR_GAME_SPAN - 1)
...
#define SP_FIAR_GAME_BORDER_SYMBOL '|'
#define SP_FIAR_GAME_FLOOR_SYMBOL '-'

#define SP_FIAR_GAME_NO_WINNER '\0'
...
/**
 * Clear the specified game to a state ready for a new game.
 * @param src - the source game
 * @return
 *	SP_FIAR_GAME_INVALID_ARGUMENT if src==NULL or src->gameHistory==NULL.
 *	SP_FIAR_GAME_SUCCESS otherwise
 */
...
/**
 * Count difference in appearences of characters at given index.
 * The count is carried out in all directions in which the given position is either
 * the left-most position or, otherwise, the lowest position.
 * Directions as returned in order:
 ...
 * @param posplayer - Player to increase the score
 * @param negplayer - Player to reduce the score
 * @param results 	- Array of size SP_FIAR_GAME_N_DIRECTIONS results will be stored in
 */
